...
export type DiscoverSearchParams = Record<
  string,
...
export function parseDiscoverSearchParams(params: DiscoverSearchParams): {
  ...
  const tab = (VALID_TABS as string[]).includes(tabRaw || "")
    ? (tabRaw as DiscoverTab)
    : "all";

  const sortRaw = first(params.sort);
  const sortBy = (VALID_SORTS as string[]).includes(sortRaw || "")
  ...
  const filters: DiscoverFilters = {
    ...
    minAmountNeeded: Number.isFinite(minAmountNeeded)
      ? minAmountNeeded
      : undefined,
    maxAmountNeeded: Number.isFinite(maxAmountNeeded)
      ? maxAmountNeeded
      : undefined,
    includeType,
    sortBy,
...
export function buildDiscoverSearchParams(
  ...
  if (filters.minAmountNeeded != null)
    params.set("minAmount", String(filters.minAmountNeeded));
  if (filters.maxAmountNeeded != null)
    params.set("maxAmount", String(filters.maxAmountNeeded));
  if (filters.includeType && filters.includeType !== "all")
    params.set("type", filters.includeType);
