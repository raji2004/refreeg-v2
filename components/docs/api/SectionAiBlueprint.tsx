    ...
    <div className="space-y-10 animate-in fade-in duration-500">
      <header className="space-y-4">
        ...
        <p className="text-slate-500 text-lg leading-relaxed">
          RefreeG is designed to be a &quot;Headless Crowdfunding Engine.&quot;
          You can integrate Large Language Models (LLMs) like GPT-4, Claude, or
          Gemini to autonomously generate and manage campaigns.
        </p>
      </header>
      ...
      <section className="space-y-8">
        <div className="p-8 border border-slate-100 rounded-3xl bg-white shadow-sm space-y-6">
          ...
          <p className="text-slate-600 text-[15px]">
            To ensure your AI agent generates valid campaign data, use the
            following template in your system instructions:
          </p>
...
ALWAYS validate the JSON structure against the /api/bot/campaigns/validate endpoint before final submission.`}
        ...
        <div className="p-8 border border-slate-100 rounded-3xl bg-slate-50 shadow-inner space-y-6">
          ...
          <p className="text-slate-600 text-[15px]">
            AI models occasionally hallucinate or miss schema constraints.
            Implement a validation loop to ensure 100% reliability:
          </p>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {[
              {
                step: "Send to /validate",
                desc: "Pass the AI-generated JSON to our validation endpoint.",
              },
              {
                step: "Handle Errors",
                desc: "If validation fails, feed the error messages back into the AI's context.",
              },
              {
                step: "Re-generate",
                desc: "Ask the AI to fix specific fields based on the validation feedback.",
              },
              {
                step: "Finalize",
                desc: "Once /validate returns success, call the /create endpoint.",
              },
            ].map((item, i) => (
              <div
                key={i}
