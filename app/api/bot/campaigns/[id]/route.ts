  ...
  try {
    ...
    if (data.bank_id || data.bank_account_number) {
      const {
        ...
        code: bankCode,
        status: bankStatus,
      } = await resolveBankDetails(authRes.userId!, authRes.mode, {
      ...
      if (bankErr || !bankDetails) {
        const response = errorResponse(
          bankErr || "Failed to resolve bank details",
          (bankCode as any) || ApiErrorCode.INTERNAL_ERROR,
          (bankStatus as any) || 500,
        );
        await logApiRequest({
          request,
          statusCode: response.status,
          ...
          userId: authRes.userId,
          mode: authRes.mode,
          errorCode: (bankCode as any) || ApiErrorCode.INTERNAL_ERROR,
          startedAt,
        });
        return response;
      }
