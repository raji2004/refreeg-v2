  ...
  try {
    ...
    if (campaign.status !== "active") {
      const response = errorResponse(
        ...
        ApiErrorCode.CAMPAIGN_NOT_ACTIVE,
        400,
      );
      await logApiRequest({
        request: req,
        ...
        userId: auth.userId,
        mode: auth.mode,
        errorCode: ApiErrorCode.CAMPAIGN_NOT_ACTIVE,
        startedAt,
      });
      return response;
    }
    ...
    if (auth.mode === "test") {
      ...
      if (testDonErr || !testDonation) {
        const response = errorResponse(
          "Failed to create test donation",
    ...
    if (!campaign.sub_account_code) {
      const response = errorResponse(
        "Campaign has no sub-account for payouts",
      ...
      await logApiRequest({
        ...
        userId: auth.userId,
        mode: auth.mode,
        errorCode: ApiErrorCode.PAYMENT_SETUP_FAILED,
        startedAt,
      });
      return response;
    }
    ...
    const paystackResponse =
      await Paystack.initializeTransaction(transactionData);
